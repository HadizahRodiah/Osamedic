...
db.serialize(() => {
  ...
  `);
 db.run(`DROP TABLE IF EXISTS  test_bookings`)

db.run(`CREATE TABLE IF NOT EXISTS test_bookings (
...
app.post('/test-booking', (req, res) => {
  ...
  const {
    patient_no, lab_no, name, sex, age, time, specimen, investigation, referredBy, date,
    tests, serology, urinalysis, biochemistry, haematology, parasitology
  } = req.body;
...
app.get('/test-bookings', (req, res) => {
  ...
  LEFT JOIN serology s ON tb.patient_no = s.patient_no
  LEFT JOIN urinalysis u ON tb.patient_no = u.patient_no
  LEFT JOIN biochemistry b ON tb.patient_no = b.patient_no
  LEFT JOIN Haematology h ON tb.patient_no = h.patient_no
  LEFT JOIN parasitologyTests p ON tb.patient_no = p.patient_no
`;
...
app.get('/masters', (req, res) => {
  const query = `
    SELECT patient_id, lab_no, name, sex, age, specimen, referred_by, date, investigation, rate, price_naira, reference_range, interpretation, remark 
    FROM printed_tests
  `;
  ...
  db.all(query, [], (err, rows) => {
    if (err) {
      ...
      return res.status(500).send('Error retrieving printed tests');
    }

    res.status(200).json(rows); // Send rows (test data) as JSON response
  });
});
